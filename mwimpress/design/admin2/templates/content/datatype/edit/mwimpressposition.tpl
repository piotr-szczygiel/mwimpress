...
{else}    {* Standard mode is browsing *}
    ...
    {else}
        ...
        <table class="list{if $attribute.content.relation_list|not} hide{/if}" cellspacing="0" id="mwImpressTable">
        <thead>
        <tr>
            <th class="tight"><img src={'toggle-button-16x16.gif'|ezimage} alt="{'Invert selection.'|i18n( 'design/standard/content/datatype' )}" onclick="ezjs_toggleCheckboxes( document.editform, '{$attribute_base}_selection[{$attribute.id}][]' ); return false;" title="{'Invert selection.'|i18n( 'design/standard/content/datatype' )}" /></th>
            <th>{'Name'|i18n( 'design/standard/content/datatype' )}</th>
            ...
            {foreach $attribute.content.relation_list as $item sequence array( 'bglight', 'bgdark' ) as $style}
              ...
              <tr class="{$style}">
                ...
                <td>
                    ...
                    <input type="hidden" id="mw_impress_{$item.contentobject_id}" name="mw_impress_position[]" value='{$item.mwimpress_position}' class="mw_impress_slides_position_{$item.contentobject_id}" />
                </td>
    ...
    <div>
        {ezscript_require('jquery-ui-1.8.22.custom/js/jquery-ui-1.8.22.custom.min.js')}
        {ezscript_require('json2.js')}
        {ezscript_require('mwimpressposition.js')}

        {ezcss_load(array('mwimpressposition.css'))}
        
        <div id="mwImpressPosition_stage">
<!--            <div class="mwImpressPosition_slide"></div>-->
        </div>
