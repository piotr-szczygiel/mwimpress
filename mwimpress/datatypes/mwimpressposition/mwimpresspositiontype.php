<?php
/**
 * File containing the mwImpressPositionType class.
 *
...
/*!
  \class mwImpressPositionType mwimpresspositiontype.php
  \ingroup eZDatatype
...
class mwImpressPositionType extends eZDataType
{
    const DATA_TYPE_STRING = "mwimpressposition";

    /*!
     Initializes with a string id and a description.
    */
    function mwImpressPositionType()
    {
        $this->eZDataType( self::DATA_TYPE_STRING, ezpI18n::tr( 'kernel/classes/datatypes', "Mw Impress Position", 'Datatype name' ),
    ...
    */
    function fetchObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        ...
        if ( $classContent['selection_type'] != 0 )
        {
            ...
            $contentObjelctAttribute->setContent( $content );
            return true;
        }
        ...
        /**** mw Impress position post handle *****/
        $mwIpressPositionPostName = "mw_impress_position";
        if($http->hasPostVariable($mwIpressPositionPostName))
        {
            $mwImpressPosition = $http->postVariable( $mwIpressPositionPostName );
            $i = 0;
            foreach($mwImpressPosition as $position)
            {
                $content['relation_list'][$i++]['mwimpress_position'] = $position;
            }
        }
        /* END mw Impress position post handle */
    ...
    function fetchClassAttributeHTTPInput( $http, $base, $classAttribute )
    {
        ...
        $postVariable = 'ContentClass_mwimpressposition_class_list_' . $classAttribute->attribute( 'id' );
        ...
        $typeVariable = 'ContentClass_mwimpressposition_type_' . $classAttribute->attribute( 'id' );
        if ( $http->hasPostVariable( $typeVariable ) )
        {
            $type = $http->postVariable( $typeVariable );
            $content['type'] = $type;
        }
        $selectionTypeVariable = 'ContentClass_mwimpressposition_selection_type_' . $classAttribute->attribute( 'id' );
    ...
    */
    ...
    static function contentObjectArrayXMLMap()
    {
        return array( 'identifier' => 'identifier',
                      ...
                      'mwimpress-position' => 'mwimpress_position'
            );
    }
    ...
    */
    function serializeContentObjectAttribute( $package, $objectAttribute )
    {
        ...
        {
            $relationItems = $relationList->getElementsByTagName( 'relation-item' );
            var_dump("serializeContentObject", $relationItems); exit;
    ...
    function postUnserializeContentObjectAttribute( $package, $objectAttribute )
    {
        ...
        for ( $i = $relationItems->length - 1; $i >= 0; $i-- )
        {
            ...
            $relationItem->setAttribute( 'mwimpress-position',  $object->attribute( 'mwimpress_position' ) );
        }
...
eZDataType::register( mwImpressPositionType::DATA_TYPE_STRING, "mwimpresspositiontype" );
